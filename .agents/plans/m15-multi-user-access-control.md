...
| **15.3** | RLS enforcement | **L** | Non-owner app role + `FORCE ROW LEVEL SECURITY` + policies; transaction wrapping per 15.0's pattern; two-role int suite with **cross-tenant negative tests**; alert-reconcile throttle / move off the SSE path (audit B.4). |
| **15.4** | RBAC | M–L | Four fixed roles + per-project grants; enforcement at route **and** policy layer; connector-approval authority (audit B.7); catalog approval + per-org firing fan-out (audit B.5, D-M15-9). |
