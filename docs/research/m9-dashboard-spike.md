...
- `@420ai/shared` is a workspace **symlink** in `node_modules`; its `package.json`
  `exports["."]` points at `./dist/index.{js,d.ts}` (a _built_ artifact).
...
export async function GET() {
  ...
  if (!res.ok) {
    return NextResponse.json(
      { error: "upstream ingest error", status: res.status },
      { status: 502 },
    );
  }
...
- **Wrap `fetch` in try/catch.** A connection refusal _throws_ (it does not return
...
"..", "..")`). Without it Next warns about multiple lockfiles / inferred root; pointing
   it at the _app_ dir breaks the build (`couldn't find next/package.json` — node_modules
...
- **`@420ai/shared` exports point at `dist`.** Bundler resolution tolerated a missing
  dist in this spike, but `transpilePackages` is the only thing that _guarantees_ it;
