...
| `pairing failed: HTTP 401` on `POST /v1/pairing-codes`       | Wrong/missing API key, or still sending `ADMIN_TOKEN`   | `ADMIN_TOKEN` was retired in M15 15.9 and now always 401s. Mint an `admin`-rung key (operations §15.9) and send that. |
| `projects is admin-gated — pass --token <apiKey>`            | You used the saved _machine_ token for an admin command | Pass `--token $API_KEY` explicitly to `collector projects`.                                                           |
...
| `SESSION_SECRET is not set` / `ARCHIVE_ENCRYPTION_KEY not configured`      | Required secret missing                                       | Generate and set them (see [install](./install.md#one-time-repo-setup-required-for-both-paths)). For the desktop app, set them in **Settings → Server config** and **Save**. |
