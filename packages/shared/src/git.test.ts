...
describe("suggestConfidence (§11.4 heuristic core)", () => {
  const cases: {
    ...
    { name: "in-window + many overlaps → medium", minutesDelta: 0, fileOverlap: 9, expected: "medium" },
    { name: "in-window + no overlap → low", minutesDelta: 10, fileOverlap: 0, expected: "low" },
    { name: "negative delta is symmetric (overlap) → medium", minutesDelta: -5, fileOverlap: 2, expected: "medium" },
    { name: "negative delta is symmetric (no overlap) → low", minutesDelta: -29, fileOverlap: 0, expected: "low" },
    { name: "exactly at the window edge → still in-window", minutesDelta: ATTRIBUTION_WINDOW_MINUTES, fileOverlap: 1, expected: "medium" },
    { name: "negative window edge → still in-window", minutesDelta: -ATTRIBUTION_WINDOW_MINUTES, fileOverlap: 0, expected: "low" },
    { name: "just past the window → null (no suggestion)", minutesDelta: ATTRIBUTION_WINDOW_MINUTES + 0.1, fileOverlap: 5, expected: null },
...
describe("GitCommitPayload shape", () => {
  it("accepts a well-formed commit literal (ISO strings, numstat, files)", () => {
    ...
    const binary: GitFileChangeLike = { path: "app-icon.png", status: "added", insertions: 0, deletions: 0 };
